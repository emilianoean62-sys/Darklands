...
// Set timeout to handle large requests
axios.interceptors.request.use(config => {
  config.timeout = 15000; // Increase timeout to 15 seconds
  return config;
});

export const runtime = 'nodejs';

// Efficient batch processing for large episode counts
async function fetchEpisodeBatches(id, fetchFunc) {
  try {
...
// Optimized episode metadata fetching
async function fetchEpisodeMeta(id, available = false) {
  try {
    ...
    // Fetch from ani.zip for optimized metadata retrieval
    ...
    const episodesArray = Object.values(episodesObj);
    
    // Handle large arrays efficiently
    if (episodesArray.length > 5000) {
      // For extremely large datasets, return a subset to prevent memory issues
      console.log(`Large episode count detected (${episodesArray.length}), returning first 5000 episodes`);
      return episodesArray.slice(0, 5000);
    }
...
// Paginated data processing for large datasets
const processDataInChunks = (data, chunkSize = 1000) => {
  if (!data || data.length === 0) return data;
  
  // For small datasets, no need to chunk
  if (data.length < chunkSize) return data;
  
  console.log(`Processing large dataset (${data.length} items) in chunks`);
